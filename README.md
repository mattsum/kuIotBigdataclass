...
chap08 선형회귀의 심화
    ...
    요약**
    01 전체-배치 경사하강법
    * 모든 훈련 데이터를 사용하여 한 번의 업데이트를 수행합니다.
    * 계산 비용이 크지만 경사 추정이 안정적입니다.
        데이터를 사용하여 실제 매개변수를 업데이트하면 다음 수식과같이 ∑기호를 포함하여경사도를 반영한다.이는 각데이터의 경사도를 모두 더해 하나의 값으로 가중치르 업데이트하는 과정이다. 이러한 방식을 일반적으로 전체 - 비치 경사하강법이라고 부르는데, 이는 모든 데이터를 한번에 입력하는 방식이다.
                                    J(θ)=m1​i=1∑m​(hθ​(x(i))−y(i))2

    02 확률적 경사하강법
    * 훈련 데이터에서 무작위로 하나의 데이터 포인트를 선택하여 매번 업데이트를 수행합니다.
    * 계산 비용이 적지만 경사 추정이 불안정하고 노이즈가 많습니다.
        확률적 경사하강법에서는 데이터셋에 있는 값들을순차적으로 추출하여 미분하는 것이 아니라 학습용 데이터에서 샘플들을 랜덤하게 뽑아서 학습할때 사용한다.

    03 미니-배치 경사하강법
    * 훈련 데이터를 작은 배치로 나누어 각 배치마다 업데이트를 수행합니다.
    * 전체 배치와 확률적 경사 하강법의 절충안으로, 계산 비용과 경사 추정의 안정성 간의 균형을 맞출 수 있습니다.
        전체 - 배치 경사하강법과 다르게 미니-배치 경사하강법은 모든 데이터를 입력하지 않고 데이터의 일부분만 입력해서 해당값들으리 경사도 평균을 구해 가중치를 업데이트 하는 방식으로, 미니 배치SGD라고 부른다.

    04 편향-분산 트레이드 오프
    * 훈련 데이터를 작은 배치로 나누어 각 배치마다 업데이트를 수행합니다.
    * 전체 배치와 확률적 경사 하강법의 절충안으로, 계산 비용과 경사 추정의 안정성 간의 균형을 맞출 수 있습니다.
        의미 : 편향과 분산간의 상충관계를 의미하는 용어이다.
        편향 : 모델의 결과가 얼마나 한쪽으로 쏠려 있는지를 나태내는 것으로, 학습된 모델이 학습 데이터에 대해 만들어낸 예측값과 실제값과의 차이이다
        분산 : 모델의 결과가 얼마나 퍼져있는지를 나타내는 것으로, 학습된 모델이 테스팅 데이터에 대해 만들어낸 예측값과 실제값과의 차이이다.

    1. L1 정규화 (Lasso Regression):

        비용 함수에 가중치의 절대값 합을 페널티로 추가합니다.
        일부 가중치를 0으로 만들어 변수 선택 효과를 제공합니다.

        J(θ)=1m∑i=1m(hθ(x(i))−y(i))2+λ∑j=1n∣θj∣
        J(θ)=m1​i=1∑m​(hθ​(x(i))−y(i))2+λj=1∑n​∣θj​∣

    2.L2 정규화 (Ridge Regression):

        비용 함수에 가중치의 제곱합을 페널티로 추가합니다.
    ...
    3.엘라스틱넷 (Elastic Net):
        ...
        L1과 L2 정규화를 결합한 방식입니다.

        J(θ)=1m∑i=1m(hθ(x(i))−y(i))2+λ1∑j=1n∣θj∣+λ2∑j=1nθj2
        J(θ)=m1​i=1∑m​(hθ​(x(i))−y(i))2+λ1​j=1∑n​∣θj​∣+λ2​j=1∑n​θj2​
    ...
    과대적합과 과소적합
        *과대적합 (Overfitting):
            모델이 훈련 데이터에 너무 잘 맞아서, 새로운 데이터에 대해서는 성능이 떨어지는 현상입니다. 이는 모델이 훈련 데이터의 노이즈나 잡음을 너무 잘 학습하여 일반화 능력이 떨어지는 경우입니다.
        *과소적합 (Underfitting):
            모델이 훈련 데이터의 패턴을 제대로 학습하지 못해 성능이 낮은 현상입니다. 모델이 너무 단순하거나 데이터의 복잡성을 충분히 반영하지 못해 학습이 부족한 상태입니다.

    L2 정규화: 리지 회귀

    * 리지 회귀 (Ridge Regression):
        L2 정규화를 사용하는 선형 회귀 방법입니다. 비용 함수에 모델 파라미터의 제곱합을 추가하여 과대적합을 방지합니다. 식으로 표현하면 다음과 같습니다:
                J(θ)=12m∑i=1m(hθ(x(i))−y(i))2+λ∑j=1nθj2
                J(θ)=2m1​i=1∑m​(hθ​(x(i))−y(i))2+λj=1∑n​θj2​
        여기서 λλ는 정규화의 강도를 조절하는 하이퍼파라미터입니다.

    * L1 정규화: 라쏘 회귀
        라쏘 회귀 (Lasso Regression):
            L1 정규화를 사용하는 선형 회귀 방법입니다. 비용 함수에 모델 파라미터의 절댓값 합을 추가하여 특성 선택을 유도하고, 과대적합을 방지합니다. 식으로 표현하면 다음과 같습니다:
                J(θ)=12m∑i=1m(hθ(x(i))−y(i))2+λ∑j=1n∣θj∣
                J(θ)=2m1​i=1∑m​(hθ​(x(i))−y(i))2+λj=1∑n​∣θj​∣
        L1 정규화는 일부 계수를 0으로 만들어 변수 선택 효과를 갖습니다.

To Sumup:

    과대적합을 방지하려면 정규화가 필요합니다.
    L2 정규화를 사용하는 리지 회귀는 모든 계수를 작은 값으로 만들지만, 일부를 0으로 만들지 않습니다.
    L1 정규화를 사용하는 라쏘 회귀는 변수 선택 효과를 통해 일부 계수를 0으로 만들어 중요한 변수만 남깁니다.


chap09 로지스틱 회귀의 기초
    01 로지스틱 회귀란?
        *예측 분석을 위한회귀분석중에서 특히 종속변수가 이분형일때 수행할 수 있는 회귀분석기법의 한종류이다.
        가설함수:
            로지스틱 회귀의 가설 함수는 시그모이드 함수(Sigmoid Function)를 사용하여 입력 변수 xx와 모델의 파라미터 ββ의 선형 결합을 확률값으로 변환합니다. 수식으로 표현하면 다음과 같습니다:
                                hβ(x)=σ(βTx)=11+e−(βTx)
                                hβ​(x)=σ(βTx)=1+e−(βTx)1​
    여기서:      βTxβTx는 입력 변수 xx와 파라미터 ββ의 내적입니다.
                σ(z)=11+e−zσ(z)=1+e−z1​는 시그모이드 함수로, zz가 입력값입니다.

    이 함수는 0과 1 사이의 값을 출력하며, 이를 통해 특정 사건이 발생할 확률로 해석할 수 있습니다.
               
        비용함수:로지스틱 회귀의 비용 함수는 로그 우도 함수(Log-Likelihood Function)를 기반으로 하며, 이를 최소화하는 방식으로 모델을 학습합니다. 로그 우도 함수는 예측값과 실제값 간의 차이를 측정하는 함수입니다. 비용 함수의 수식은 다음과 같습니다:
                        J(β)=−1m∑i=1m[y(i)log⁡(hβ(x(i)))+(1−y(i))log⁡(1−hβ(x(i)))]
                        J(β)=−m1​i=1∑m​[y(i)log(hβ​(x(i)))+(1−y(i))log(1−hβ​(x(i)))]

    여기서: mm은 샘플의 개수.
                        y(i)y(i)는 ii-번째 샘플의 실제 값 (0 또는 1).
                        hβ(x(i))hβ​(x(i))는 ii-번째 샘플에 대한 예측 확률입니다.

    이 비용 함수는 모델이 예측한 확률과 실제 값이 얼마나 일치하는지를 측정하며, 이를 최소화하는 파라미터 ββ를 찾습니다.

        *혼동행렬
            예측값이 실제값 대비 얼마나 잘 맞는지 2X2 행렬로 표현하는 기법이다.
            혼동 행렬은 분류 모델의 성능을 평가하는 데 사용되는 표입니다. 이 행렬은 실제 클래스와 모델이 예측한 클래스를 비교하여 다음과 같은 네 가지 지표를 제공합니다:
            True Positive (TP): 실제 양성인 경우를 모델이 양성으로 예측한 수.
            True Negative (TN): 실제 음성인 경우를 모델이 음성으로 예측한 수.
            False Positive (FP): 실제 음성인 경우를 모델이 양성으로 잘못 예측한 수 (거짓 양성).
            False Negative (FN): 실제 양성인 경우를 모델이 음성으로 잘못 예측한 수 (거짓 음성).

    02 분류 문제의 성능지표
        정확도 : 전체 데이터 개수 대비 정답을 맞춘 데이터의 개수
        정밀도 : 모델이 1이라고 예측했을때 얼마나 잘 맞을지에 대한 비율
        민감도 : 실제 1인 값을 가진 데이터를 모델이 얼마나 1이라고 잘 예측했는지에 대한 비율
        F1 스코어 : 정밀도와 민감도를 통합한 지표로 정밀도와 민감도의 조화평균값
...
chap10 로지스틱 회귀의 심화
    ...
    다중 클래스 분류 (Multiclass Classification)
        ...
        로지스틱 회귀는 기본적으로 이진 분류를 위해 설계되어 있습니다. 그러나 다중 클래스 문제를 해결하기 위해 몇 가지 접근 방법이 있습니다.

        *One-vs-Rest (OvR) 접근법

        가장 흔히 사용되는 방법 중 하나는 One-vs-Rest (OvR) 방식입니다. 이 방법은 각 클래스에 대해 이진 분류기를 훈련시키는 방식입니다. 예를 들어, KK개의 클래스가 있을 때, 총 KK개의 모델을 훈련시키며 각 모델은 하나의 클래스와 나머지 클래스들을 구분합니다.

        * 클래스 kk에 대한 모델: P(y=k∣x)P(y=k∣x)를 학습.
        * 나머지 클래스: P(y≠k∣x)P(y=k∣x).

        소프트맥스 회귀 (Softmax Regression)

        소프트맥스 회귀는 다중 클래스 분류를 위해 로지스틱 회귀를 확장한 방법입니다. 손실 함수는 크로스 엔트로피 손실 함수로 정의됩니다:
                    J(β)=−∑i=1m∑k=1Kyk(i)log⁡(eβkTx(i)∑j=1KeβjTx(i))
                    J(β)=−i=1∑m​k=1∑K​yk(i)​log(∑j=1K​eβjT​x(i)eβkT​x(i)​)

        여기서:
                * yk(i)yk(i)​는 ii-번째 샘플의 실제 클래스가 kk일 때 1, 그렇지 않으면 0.
                * x(i)x(i)는 ii-번째 샘플의 특징 벡터입니다.

    02 다중클래스 분류를 코드로 구현하기

    03 ROC 커브와 AUC
        * ROC 곡선: 다양한 임계값에서 TPR과 FPR을 시각화.
        * AUC: ROC 곡선 아래 면적으로, 0.5에서 1 사이의 값을 가지며, 값이 클수록 모델 성능이 좋음.


    **요약**
    01 다중클래스 분류
        정의 : 2개 이상의 클래스를 가진 y값에 대한 분류이다
        다중클래스 분류와 다중레이블 분류의 차이점
        -다중클래스 분류 : 2개 이상의 클래스를 가진 분류 작업
        -다중레이블 분류 : 상호 배타적이지 않은 속성 예측
        다중클래스 분류 문제의 두가지 접근법
        -One-vs-All : m개의 클래스가 존재할 때 각 클래스마다 분류기를 생성하여 분류하는 기법
        -One-vs-One : m개의 클래스가 있다면, 이 클래스의 분류기를 하나으 ㅣ클래스로 하고 나머지 클래스의 분류기들을 만들어 최종적으로 각 분류기들의 결과를 투표로 결정하는 기법

    02 소프트맥스 함수
        - 다중클래스 분류에서 여러 선형회귀의 출력 결과를 정규화하여 합이 1이 되도록 만드는 함수

    03 다중클래스 분류를 코드로 구현하는 방법
        데이터셋선정 -> 데이터 호출 -> 데이터 분류 -> 모델생성 -> 성능측정

    04 ROC커브와 AUC
        ROC커브 : 분류기의 임계값을 지속적을 조정하여 정밀도와 민감도 간의 비율을 도식화하여 표현하는 기법중 하나이다.
        AUC : ROC커브 하단의 넓이로, 이 값을 사용하여 모델의 성능 측정하기도한다.
--------------------------------------------------------------------------------
## 2024-06.12 [수]
--------------------------------------------------------------------------------
...
Part4 고급머신러닝
    ...
    *나이브 베이지안 분류기는 베이즈 정리에 기반한 분류 기법입니다. 이 방법은 독립 변수들 간의 독립성을 가정하며, 특정 클래스에 속할 확률을 계산하는 데 사용됩니다. 나이브 베이지안 분류기는 텍스트 분류, 스팸 필터링, 감정 분석 등 다양한 분야에서 널리 사용됩니다.
    
        01 베이즈 정리의 이해
            베이즈 정리 (Bayes' Theorem)

            베이즈 정리는 조건부 확률을 계산하는 데 사용되는 중요한 수학적 정리입니다. 베이즈 정리는 다음과 같이 표현됩니다:
            P(A∣B)=P(B)P(B∣A)⋅P(A)​
        
        02 베이즈 분류기 구현하기
        03 나이브 베이지안 분류기 구현하기
        04 20 newsgroup으로 분류 연습하기

    나이브 베이지안의 장점과 단점
        장점
            계산 효율성: 간단하고 빠르며, 훈련과 예측이 모두 효율적입니다.
            적은 데이터로도 학습 가능: 적은 양의 데이터를 가지고도 효과적으로 작동합니다.
            높은 차원의 데이터에 적합: 고차원의 데이터에서도 비교적 잘 작동합니다.

        단점
            독립성 가정의 한계: 특성 간의 상관관계를 무시하기 때문에, 실제 데이터가 이 가정을 만족하지 않으면 성능이 떨어질 수 있습니다.
            데이터 표현의 제약: 연속형 데이터는 가우시안 분포를 가정해야 하며, 이산형 데이터는 베르누이 또는 멀티노미얼 분포를 가정해야 합니다.

        나이브 베이지안 분류기는 그 단순함과 효율성 덕분에 다양한 문제에 적용될 수 있으며, 특히 초기 모델로서 좋은 성능을 보여줍니다. 특성 간의 독립성이 엄밀히 만족되지 않더라도 실용적으로 유용한 결과를 얻을 수 있는 경우가 많습니다.

    **요약**
    1. 확률의 표현
        *이산형 값 : 전체 표본공간에서 가능한 모든결과 횟수대비 해당사건의 결과횟수에 대한 비율을 표시한다.
        *연속형 값 : 해당 데이터를 적절히 표현하는 함수를 생성한 후 해당 함수의 적분값을 취한다.
        *조건부 확률 : 어떤사건이 일어난다고 가정했을때 다른사건이 일어날 확률을 의미한다.
    2.베이즈 정리(Bayes's Theorem)
        *두확률 변수의 사전확률과 사후확률 사이의 관계를 나타내는 정리이다. 베이지 정리의 기본적인 전제는 객관적인 확률이 존재하지 않는다는 것이다.
    3.나이브 베이지안 분류기(Naive Bayes Classifier)의 종류
        *베르누이 분류기(BernoulliNB) : 기본적으로 다루고자 하는 모든데이터가 불린 피쳐(boolean feature)이다. 베르누이 분류기에 사용되는 데이터 타입은 이산형 데이터인데, 이러한 데이터를 모두 불린 타입으로 변경하여 학습한다.
        *다항 분류기(MultinomialNB):베르누이 분류기와 달리 각 피쳐들이 이산형이지만, 이진값이 아닌 여러개의 값을 가질 수 있다. 이러한 경우 앞서 보았던 나이브 베이지안 식을 변형하여 사용한다.
        *가우시안분류기(GaussianNB):연속형 값을 피쳐로 가진 데이터에 확률을 구하기 위해 y의 분포를 정규분포(gaussian)로 가정한다. 이에따라 확률 밀도 함수 상의 해당 값x가 나올 확률로 나이브 베이지안(NB)을 구현하여 사용한다.
    4.뉴스분류기 모델 개발의 전략
        데이터준비 -> 데이터 전처리 -> 백터화하기 -> 학습 -> 모델결정
    5.모델개발의 추가기법
        *백터화하기(vectorizer):CountVectorizer를 약간 변형한 TF-idfVectorizer을 생성하여 텍스트 백터화 시키는 방법이다.
        *교차검증(cross validation):성능을 측정할떄 훈련 데이터셋과 테스트 데이터셋으로 나누어 여러번 측정하여 모델 성능의 평균치를 측정하는 기법이다.
        *파이프라인(pipeline): 단순히 모델개발에 그치지않고 일종의 제품을 개발할때 쓰는기법으로, 데이터전처리부터 성능 측정까지 연결된 코드로 나타내는 것을 말한다. 파이프라인을 사용하면 입력(input)으로 일관된 출력(output)을 뽑을 수 있다는 장점이 있다.

    Chap12 의사결정트리
        01 의사결정트리의 이해
            의사결정트리(Decision Tree)는 분류 및 회귀 작업에 사용되는 지도 학습 알고리즘입니다. 데이터를 특징(feature)으로 나누어 결정을 내리는 트리 구조를 사용하여 목표 변수의 값을 예측합니다. 의사결정트리는 이해하기 쉽고 시각화가 용이하여 데이터 분석 및 해석에 유용합니다.
        02 의사결정트리 알고리즘
            의사결정트리 알고리즘은 데이터의 특징들을 기반으로 분할하여 예측을 수행하는 트리 구조의 지도 학습 알고리즘입니다. 주요 알고리즘으로는 ID3, C4.5, CART 등이 있습니다. 각 알고리즘은 트리를 성장시키고, 적절한 특징을 선택하는 기준이 다릅니다. 아래에서 의사결정트리 알고리즘의 작동 원리와 주요 알고리즘을 설명하겠습니다.
        의사결정트리의 작동 원리
            1.특징 선택: 가장 중요한 특징을 선택하여 데이터를 분할합니다. 분할 기준으로는 정보 이득(information gain), 지니 계수(Gini index) 등을 사용합니다.
            2.분할: 선택한 특징의 값에 따라 데이터를 두 개 이상의 그룹으로 나눕니다.
            3.반복: 분할된 각 그룹에 대해 다시 특징을 선택하고 분할하는 과정을 반복합니다.
            4.종료 조건: 더 이상 분할이 불가능하거나 사전에 정의한 조건에 도달할 때까지 위 과정을 반복합니다. 예를 들어, 최대 트리 깊이에 도달하거나 리프 노드에 있는 데이터의 수가 최소 샘플 수보다 작아질 때 멈춥니다.
        03 의사결정트리의 확장
            의사결정트리의 확장은 여러 방향으로 이루어질 수 있으며, 이를 통해 모델의 성능을 향상시키고 다양한 문제에 적용할 수 있습니다. 주요 확장 방법으로는 앙상블 방법, 랜덤 포레스트, 그래디언트 부스팅, XGBoost 등이 있습니다.
            1. 앙상블 방법 (Ensemble Methods)

            앙상블 방법은 여러 개의 의사결정트리를 결합하여 단일 모델보다 더 나은 성능을 얻는 기법입니다. 대표적인 앙상블 방법으로는 배깅(Bagging)과 부스팅(Boosting)이 있습니다.
            
            *배깅 (Bagging)
            배깅은 여러 모델을 독립적으로 학습시키고, 그 예측을 평균내거나 투표하여 최종 예측을 만드는 방법입니다. 랜덤 포레스트가 배깅의 대표적인 예입니다.

                -랜덤 포레스트 (Random Forest): 여러 개의 의사결정트리를 생성하고 각 트리는 데이터의 무작위 샘플과 무작위 특징 하위 집합을 사용하여 학습됩니다. 최종 예측은 모든 트리의 예측을 평균내거나 다수결 투표로 결정합니다.

            *부스팅 (Boosting)
            부스팅은 모델을 순차적으로 학습시키고, 이전 모델의 오류를 수정하는 방식으로 성능을 향상시키는 방법입니다. AdaBoost, Gradient Boosting, XGBoost 등이 부스팅의 대표적인 예입니다.

                -그래디언트 부스팅 (Gradient Boosting): 이전 모델의 오류를 보정하는 방식으로 새로운 모델을 학습시키는 알고리즘입니다.
                -XGBoost (Extreme Gradient Boosting): 그래디언트 부스팅의 확장판으로, 효율성과 성능을 크게 향상시킨 알고리즘입니다.
            
            2. 하이퍼파라미터 튜닝
                의사결정트리와 앙상블 모델의 성능을 최적화하기 위해 하이퍼파라미터 튜닝을 수행할 수 있습니다. 그리드 서치(Grid Search)와 랜덤 서치(Random Search)가 일반적인 방법입니다.

                *그리드 서치 (Grid Search): 미리 정의된 하이퍼파라미터 값들의 조합을 모두 시도하여 최적의 조합을 찾습니다.
                *랜덤 서치 (Random Search): 하이퍼파라미터 공간에서 무작위로 조합을 선택하여 최적의 조합을 찾습니다.

        04 의사결정트리 알고리즘의 다양한 변형
            의사결정트리의 다양한 확장 기법은 모델의 성능을 향상시키고 특정 문제에 더 효과적으로 적용될 수 있도록 도와줍니다. 여기서는 트리 가지치기, 연속형 변수 분할, 희귀 트리에 대해 설명하겠습니다.

            1. 트리 가지치기 (Tree Pruning)
            트리 가지치기는 의사결정트리의 과적합을 방지하고 모델을 더 일반화하기 위해 불필요한 가지를 제거하는 과정입니다. 가지치기는 사전 가지치기(pre-pruning)와 사후 가지치기(post-pruning)로 나뉩니다.

            *사전 가지치기 (Pre-pruning)
            사전 가지치기는 트리 생성을 중간에 멈추는 방식으로, 노드 분할을 멈출 조건을 미리 설정합니다.
                -최대 깊이 (max_depth): 트리의 최대 깊이를 제한합니다.
                -최소 샘플 수 (min_samples_split): 노드를 분할하기 위한 최소 샘플 수를 설정합니다.
                -최소 잎 노드 샘플 수 (min_samples_leaf): 리프 노드가 되기 위한 최소 샘플 수를 설정합니다.
            
            *사후 가지치기 (Post-pruning)
            사후 가지치기는 트리를 완전히 생성한 후, 불필요한 노드를 제거하여 가지를 치는 방법입니다. 대표적인 방법은 비용 복잡도 가지치기 (Cost Complexity Pruning)입니다.

                -비용 복잡도 가지치기 (Cost Complexity Pruning): 트리의 각 가지에 대한 복잡도 매개변수를 사용하여 가지를 치는 방법입니다.

            2. 희귀 트리 (Rare Event Decision Trees)
            희귀 트리는 희귀한 이벤트를 다루기 위해 설계된 트리입니다. 일반적인 의사결정트리와 달리, 희귀 이벤트의 예측 성능을 높이기 위해 특수한 방법을 사용합니다.        
            *재샘플링 (Resampling): 희귀 이벤트를 다루기 위해 데이터의 샘플링 비율을 조정합니다. 오버샘플링(oversampling) 또는 언더샘플링(undersampling)을 사용하여 클래스 불균형 문제를 해결합니다.
        
        05 의사결정트리의 구현

chap13 앙상블
    01 앙상블의 이해
        -여러개의 알고리즘들이 하나의 값을 예측하는 기법을 통칭하여 말하는 것으로, 이는 각 알고리즘이 Y값에 대한 투표를 실시하여 해당 Y값을 예측하는 기법이라고 이해할 수 있다.

        *바닐라 앙상블(vanila ensemble): 가장 기본적인 앙상블기법이다. 보통 바닐라라고 하면 아이스크림에서 아무것도 첨가되지 않은 맛을 의미하는데, 바닐라 앙상블의 의미도 동일하게 아무것도 처리하지않은 앙상블모델을 의미한다. 일반적으로 가중치 평균이나 투표에 의한 방식으로 만들어지는 앙상블 모델이다.

    02 투표분류기
        -앙상블 모델의 가장 기본적인 형태로 다수결분류기(majority voting classifier)라고도 부른다. 여러개의 모델을 다양하게 만든 후, 해당 모델에 모두 같은 데이터를 넣고 모델들의 결과를 취합하여 가장 많이 선택된 결과를 취하는 것이 가장 기본적인 형태이다.

    03 배깅과 랜덤포래스트

        *배깅
        -하나의 데이터셋에서 샘플링을 통해 여러개의 데이터셋을 만든 다음 각 데이터셋마다 모델을 개발하여 투표분류기(voting classifier)로 만드는 기법이다.
        
        *랜덤포레스트
        -랜덤하게 숲을 생성하는 알고리즘으로, 배깅 알고리즘을 의사결저트리에 적용한 모델이다. 즉 하나의 모델을나무라고 한다면 이러한 나무들을 이용해 랜덤하게 데이터를 뽑아서 여러개의 모델을 만들었을때 이를 하나의 숲이라고 할 수 있다.

        *부트스트래핑
        -모수 데이터로부터 학습데이터를 추출할때 임의의 데이터를 추출한 후 복원 추출하는 여러번의 과정을 의미한다.

    04 부스팅
        -학습 라운드를 차례로 진행하면서 각 예측이 틀린 데이터에 점점 가중치를 주는 방식, 즉 라운드별로 모델을 만들어 그 모델들의 결과에 가중치를 주어 앙상블 모델을 만드는 방식이다.
